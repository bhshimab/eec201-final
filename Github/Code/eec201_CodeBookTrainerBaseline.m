function [codebook, fileorder] = eec201_CodeBookTrainerBaseline(training_folder_path, show_work)
%
%
      ...
      % If the file is valid
        if and(~filelist(ind).isdir, ~strcmp(filelist(ind).name(1), '.'))
          ...
          % Do the VQ training to produce the vector codebook to compare unknowns to
            codebook{cbInd} = eec201_LBGVQTrainer(      melfreqCept(2:N_MFCOEFFS+1, :),                     ...
                                                        CB_SIZE,                                            ...
                                                        CB_DELTA);
