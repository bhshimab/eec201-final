function [codebook] = eec201_LBGVQTrainer(training_vectors, codebook_size, delta, show_work)
...
%
 ...
 %% Algorithm Initilization -----------------------------------------------------------------------------------
  ...
  % This represents the 'center of mass' along each dimension of all of the vectors
    codebook = mean(training_vectors, 2)';

  % n_vectors is the number of ceptstrum frames in time
    [nDims, nVectors] = size(training_vectors);
    
  % Setup fancy figure for gif plotting
    if SHOW_WORK
        F = figure('Position', [1000, 300, 1200, 1200], 'Color', [1,1,1]);
        T = tiledlayout(F, 3, 1);
    ...
    while length(codebook(:, 1)) < codebook_size
      ...
      % forward and backwards a small amount in each dimension
        ...
        if SHOW_WORK
            ...
            scatter3(G, training_vectors(1, :), training_vectors(2, :), training_vectors(3, :), '.');
            scatter3(G, codebook(end-1, 1), codebook(end-1, 2), codebook(end-1, 3), 64, 'red', '*');
            scatter3(G, codebook(end,   1), codebook(end,   2), codebook(end,   3), 64, 'red', '*');
            view(G, 45, 45)
            xlim(G, [-1.5, 1.5]); ylim(G, [-1.5, 1.5]); zlim(G, [-1.5, 1.5]);
            A  = animatedline(H);
            animIter = 1;
        end %if
        ...
        while BreakFlag
          ...
          % Guess new codewords as the center of mass of only the near vectors
            for ind = 1:nCodewords
                if any(cells==ind)
                    codebook(ind, :) = mean(training_vectors(:, cells==ind), 2);
                else
          ...
          % Fancy GIF animation of the training/centroid process
            if SHOW_WORK
                ...
                for ind = 1:nCodewords
                    scatter3(G, codebook(ind, 1), codebook(ind, 2), codebook(ind, 3), 64, 'red', '*');
                end %for ind
