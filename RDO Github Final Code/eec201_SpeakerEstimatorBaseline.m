function eec201_SpeakerEstimatorBaseline(speaker_folder_path, codebook, codepops, codeinds, trainorder, show_work)
          ...
          % For each trained VQ codebook:
            for jnd = 1:length(codebook)
              % Calculate the minimum frame-to-codeword distances per codebook
                [distances(jnd, :),  ...
                 confidence(jnd)]    = eec201_CalcSpeakerLikeness(mfcc(:, 2:N_MFCOEFFS+1),                  ...
                                                                  codebook{jnd},                            ...
                                                                  codepops{jnd},                            ...
                                                                  codeinds{jnd});
            end %for ind

            [~, guess] = min(mean(distances, 2)'); % ./ confidence);
